"""
This file contains the class ActorCriticAttackTwoLayer which calls Q functions based on a linear neural net
...
class CriticModel():
	"""
	Class holds the critic network for a2c
	"""
	def __init__(self, nS, save_path, restore_path, learning_rate, batch_size):
		"""
		Creates a session of the model defined here
		"""
		self.nS = nS
		...
		self.output = tf.layers.dense(inputs=self.dense2, units = 1, activation=None, use_bias=False, name='output')

		self.loss = tf.nn.mean_squared_error(labels=self.labels, predictions=self.output)
...
class ActorCriticAttackTwoLayer():
	"""
...
################### WARNING!! CHANGE THIS WHEN MAKING NEW NETWORK!!!#################
		self.module_string = 'actor_critic_attack_two_layer'
		...
		self.pre_mask_output = tf.layers.dense(inputs = self.dense2, units = self.nA, activation = None, use_bias = False, name = 'output')
...
################### Determine how best to return q function ##########
	...
	def get_action(self, state_vector, valid_mask, update=None, action_taken=None, target=None, loss_weights=None):
		...
		soft_q = utils.softmax_valid(q)
		if update:
			action = utils.choose_by_weight(soft_q)
		else:
			action = np.argmax(soft_q)
		return action
