...
################### WARNING!! CHANGE THIS WHEN MAKING NEW NETWORK!!!#################
		self.module_string = 'leaky_relu_3_layer'
		...
		# First hidden Layer
		self.dense1 = tf.layers.dense(inputs = self.features, units = 32, activation = tf.nn.leaky_relu, use_bias = True, name = 'dense1')
		self.dense2 = tf.layers.dense(inputs = self.dense1, units = 16, activation = tf.nn.leaky_relu, use_bias = True, name = 'dense2')
		self.dense3 = tf.layers.dense(inputs = self.dense2, units = 8,  activation = tf.nn.leaky_relu, use_bias = True, name = 'dense3')
